...
def evaluate_actions(args, model, tokenizer, image_size, use_thumbnail):
    ...
    with torch.no_grad():
        for _, (pixel_values, questions, gts, ids, pixel_labels) in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            evaluate_text_action(pred_text, gts[0])
    ...
    if torch.distributed.get_rank() == 0:
        print("\n" + "-" * 60)
        print("PIXEL-HEAD METRICS")
        print("-" * 60)
        print(f"Total predictions: {pixel_eval_summary['total']}")
        print(f"Valid predictions: {pixel_eval_summary['valid']}")
        for thr in PIXEL_EVAL_THRESHOLDS:
            hits = pixel_eval_summary['hits'][thr]
            acc = pixel_eval_summary['accuracy'][thr]
            print(f"Acc@{thr:.2f}: {acc:.4f} ({hits}/{pixel_eval_summary['valid']})")
        print("-" * 60 + "\n")
