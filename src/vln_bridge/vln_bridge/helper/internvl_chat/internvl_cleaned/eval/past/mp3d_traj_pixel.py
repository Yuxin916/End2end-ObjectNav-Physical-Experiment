...
def draw_prediction_square(image, center_x, center_y, color, half_size=5):
    """Draw a small square marker while clipping to image bounds."""
    h, w = image.shape[:2]
    if h == 0 or w == 0:
        return
    x0 = max(center_x - half_size, 0)
    x1 = min(center_x + half_size, w)
    y0 = max(center_y - half_size, 0)
    y1 = min(center_y + half_size, h)
    if x0 >= x1 or y0 >= y1:
        return
...
def _count_to_word(count: int) -> str:
    """Convert small integers to words for human-friendly summaries."""
    return NUMBER_WORDS.get(count, str(count))


def format_action_history(actions: List[str]) -> str:
    """Summarize consecutive actions as 'three turn_left -> two move_forward'."""
    if not actions:
    ...
    for action in actions[1:]:
        if action == prev_action:
            run_length += 1
            continue
        parts.append(f"{_count_to_word(run_length)} {prev_action}")
        prev_action = action
        run_length = 1
...
class VLMPolicy:
    ...
    def construct_prompt(self, instruction, rgb_img_padded, occupancy_single_color=None,history_5Interval1=None, history_5Interval3=None, history_5Interval5=None,
        ...
        if self.template == "RGB":
        ...
        elif self.template == "RGB_HisKFnoColor":
            if fpv_imgs_no_draw is None: # 
                raise ValueError("HisKFnoColor must be provided for RGB_HisKFnoColor template")
            images = [rgb_img_padded] + fpv_imgs_no_draw
            question = f"{introduction}Instruction: {instruction}\n\nInput: {RGB_PADDED_DOWN}{HIS_KEYFRAME_NO_COLORED}\nOutput: \n<GOAL_PIXEL_X>, <GOAL_PIXEL_Y>\n"
            ...
            images = [rgb_img_padded] + fpv_imgs_single_colored
            question = f"{introduction}Instruction: {instruction}\n\nInput: {RGB_PADDED_DOWN}{HIS_KEYFRAME_SINGLE_COLORED}{HIS_FULL_ACTION}{history_action_text}\nOutput: \n<GOAL_PIXEL_X>, <GOAL_PIXEL_Y>\n"
        else:
...
def evaluate_trajectories(vlm_policy, config):
    ...
    for ep_idx in tqdm(range(total_episodes), desc="Evaluating episodes"):
        ...
        fpv_imags_multi_colored, fpv_imgs_blue, fpv_imgs_no_draw, occupancy_colored = None, None, None, None
        ...
        elif vlm_policy.template in ["RGB_HisKFMultiColor_OccMultiColor", "RGB_HisKFMultiColor", "RGB_HisKFSingleColor", "RGB_HisKFnoColor", "RGB_HisKFSingleColor_HisAction", "RGB_HisKFMultiColor_OccSingleColor"]:
            pass
        else:
        ...
        history_buffer.append({'rgb': obs['rgb'], 'depth': rgbd[3:4, :, :].transpose(1, 2, 0),  'pose': current_pose})
        ...
        while not done and step_idx < max_steps:
            ...
            elif vlm_policy.template in ["RGB_HisKFMultiColor_OccMultiColor", "RGB_HisKFMultiColor", "RGB_HisKFSingleColor", "RGB_HisKFnoColor", "RGB_HisKFMultiColor_OccSingleColor", "RGB_HisKFSingleColor_HisAction"]:
            ...
            if VEBOSE:
                print(f"text outpt: {text_output};Raw pixel output from VLM: {pixel_value}")
            pixel_output = pixel_value.reshape(-1, 2)
            inference_times.append(time.time() - inference_start)


            # ---------------------- Execute Action ---------------------- #
            if len(pixel_output) == 1:
                pixel_value = pixel_output[0]  # (x, y) normalized
            elif len(pixel_output) > 1:
                pixel_value = pixel_output[len(pixel_output)-2] 

            if vlm_policy.output_template == "pixel":
                text_output = "SELECT_PIXEL"
            else:
                raise NotImplementedError(f"Output template {vlm_policy.output_template} not implemented for action execution")
            
            
            X, Y = int(pixel_value[0]*IMG_W), int(pixel_value[1]*IMG_H) # X, Y
            if VEBOSE:
            ...
            # Draw every candidate pixel (green) and highlight the executed one (red).
            if torch.is_tensor(pixel_output):
                pixel_candidates = pixel_output.to(dtype=torch.float32).detach().cpu().numpy()
            else:
                pixel_candidates = np.asarray(pixel_output, dtype=np.float32)
            for px, py in pixel_candidates:
                draw_prediction_square(rgb_pixel, int(px * IMG_W), int(py * IMG_H), (0, 255, 0))
            draw_prediction_square(rgb_pixel, X, Y, (0, 0, 255))
            ...
            if X >= PADDING and X <= (256 + PADDING) and Y < 256:  # move toward pixel
                ...
                obs, rgbd, done, infos, steps_taken, episode_done, last_infos, actions, step_idx, \
                    episode_data, history_buffer, vis_data, episode_rgb_history, if_first_stop = follow_toward_waypoint(
                        ...
                        obs=obs, rgbd=rgbd, infos=infos, done=done, policy_agent=vlm_policy, history_buffer=history_buffer, action_history=action_history, current_episode=current_episode, 
                        device=device, vis_data=vis_data,
                        config=config, selected_pixel=selected_pixel, episode_rgb_history=episode_rgb_history, history=history,
                    )
...
def parse_vlm_args():
    ...
    parser.add_argument("--template", type=str,
                        choices=["RGB", "RGB_His5Interval1", "RGB_His5Interval3", "RGB_His5Interval5", 
                                 ...
                                 "RGB_HisKFnoColor", "RGB_HisKFMultiColor", "RGB_HisKFSingleColor_HisAction",
                                 "RGB_HisKFMultiColor_OccMultiColor", "RGB_HisKFSingleColor", "RGB_HisKFMultiColor_OccSingleColor"],
                        help="Training template used for the model")
    parser.add_argument("--output_template", type=str, default="text_pixel",
                    choices=["pixel", "text_pixel"], help="Output template used for the model")
