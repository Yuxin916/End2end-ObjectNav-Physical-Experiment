...
from internvl_cleaned.model import load_model_and_tokenizer, load_GO2PIXEL_model_and_tokenizer
...
def seconds_to_hours(total_seconds: float) -> float:
    """Convert seconds to hours with safe finite fallback."""
    return safe_metric(total_seconds) / 3600.0
...
class VLMPolicy:
    ...
    def get_pixel(self, instruction, rgb_img, rgb_img_padded, generation_config, occupancy_single_color=None,
                  ...
                  occupancy_colored=None,history_action_text=None, history_traj_mask=None,
    ):
...
def evaluate_trajectories(vlm_policy, config):
    ...
    for ep_idx in tqdm(range(total_episodes), desc="Evaluating episodes"):
        ...
        planner_pose_inputs = build_surrogate_planner_pose_inputs(obs.get("world_pos", agent.start_position), obs.get("world_rotation", agent.start_rotation))
        history.reset(obs.get("world_pos", agent.start_position), obs.get("world_rotation", agent.start_rotation), rgbd[:, :, 0:3], rgbd[:, :, 3], planner_pose_inputs)
        ...
        while not done and step_idx < max_steps:
            ...
            if vlm_policy.template in ["RGB_His5Interval1", "RGB_His5Interval1_OccSingleColor"]:
                ...
                # Keep history order consistent with data writer: most recent -> oldest.
                fpv_imags_multi_colored = list(reversed(fpv_imags_multi_colored))
                fpv_imgs_blue = list(reversed(fpv_imgs_blue))
                fpv_imgs_no_draw = list(reversed(fpv_imgs_no_draw))
                ...
                # only get single color history images
                ...
                if args.use_trajectory_embeddings and fpv_imgs_blue:
                    history_mask_debug_root = None
                    if "his_KF_single_color" in agent.paths:
                        history_mask_debug_root = Path(agent.paths["his_KF_single_color"]).parent / "his_KF_single_color_mask_debug"
                    elif "rgb" in agent.paths:
                        history_mask_debug_root = Path(agent.paths["rgb"]).parent / "his_KF_single_color_mask_debug"

                    history_mask_debug_step_dir = None
                    if history_mask_debug_root is not None:
                        history_mask_debug_step_dir = history_mask_debug_root / f"{step_idx:05d}"

                    history_token_masks = build_history_trajectory_token_masks(
                        fpv_imgs_blue,
                        grid_size=16,
                        token_pixels=16,
                        min_blue_pixels=1,
                        blue_tol=25,
                        debug_save_dir=(str(history_mask_debug_step_dir) if history_mask_debug_step_dir is not None else None),
                    )
                    if history_token_masks.size > 0:
                        current_mask = np.zeros((1, history_token_masks.shape[1]), dtype=np.int64)
                        full_masks = np.concatenate([current_mask, history_token_masks], axis=0)
                        history_traj_mask = torch.as_tensor(full_masks, dtype=torch.long, device=vlm_policy.device)
                ...
                if ref_path:
                    def rotate_toward(waypoint_target):
                        ...
                        while not done:
                            ...
                            history.update(obs["world_pos"], obs["world_rotation"], action_id, rgbd[:, :, 0:3].astype(np.uint8), rgbd[:, :, 3], planner_pose_inputs)
                    ...
                    # After GT heading warmup, force forward steps before VLM takes over.
                    for _ in range(0):
                        if done:
                            break
            ...
            text_output = vlm_policy.get_pixel(
                ...
                occupancy_colored=None,
                history_action_text=history_action_text,
                history_traj_mask=history_traj_mask,
            )
            inference_times.append(time.time() - inference_start)

            # print(f"time_step: {step_idx}, text_output: {text_output}")
            
            # ---------------------- Execute Action ---------------------- #
            if vlm_policy.output_template == "pixelintext":
                agent, envs, follower, step_idx, episode_data, obs, rgbd, infos, done, vlm_policy, current_episode, vis_data, episode_rgb_history, history, action_history, vis_config = output_template_pixelintext_correct(
                    agent, envs, follower, step_idx, episode_data, obs, rgbd, infos, done, vlm_policy, current_episode, vis_data, episode_rgb_history, history, action_history, vis_config,
                    text_output,  VEBOSE, PADDING, WRITE_VISUALIZE, device, config, img_W, img_H, IMG_W, IMG_H, history_images=fpv_imgs_blue)
            elif vlm_policy.output_template == "action":
                envs, agent, episode_rgb_history, history, current_episode, episode_data, config, device, vis_config, done, step_idx, action_history = output_template_action(
                        text_output, envs, agent, episode_rgb_history, history, current_episode, episode_data,
                        config, device, vis_config, done, step_idx, action_history, action_labels,
                        WRITE_VISUALIZE, VEBOSE, DEBUG_SAVE_PLY, history_images=fpv_imgs_blue)
            elif vlm_policy.output_template == "action_pixel":
                agent, envs, follower, step_idx, episode_data, obs, rgbd, infos, done, vlm_policy, current_episode, device, vis_data, config, episode_rgb_history, history, action_history = output_template_action_pixel(
                                text_output=text_output, agent=agent, envs=envs, follower=follower, step_idx=step_idx, episode_data=episode_data, obs=obs, rgbd=rgbd, infos=infos, done=done, vlm_policy=vlm_policy, current_episode=current_episode, device=device, vis_data=vis_data, config=config, episode_rgb_history=episode_rgb_history, history=history, action_history=action_history,
                                vis_config=vis_config, img_W=img_W, img_H=img_H, IMG_W=IMG_W, IMG_H=IMG_H, PADDING=PADDING, WRITE_VISUALIZE=WRITE_VISUALIZE, VEBOSE=VEBOSE, DEBUG_SAVE_PLY=DEBUG_SAVE_PLY, history_images=fpv_imgs_blue)
            else:
                raise ValueError(f"Unknown output template: {vlm_policy.output_template}")
            
        # Compute average inference time
        avg_inference_time = sum(inference_times) / len(inference_times) if inference_times else 0.0
        episode_elapsed_hours = seconds_to_hours(time.time() - episode_start_time)
        # Log episode summary
        ...
        habitat_logger.info(
            ...
            f"Elapsed: {episode_elapsed_hours:.4f}h, "
        ...
        episode_result = {
            ...
            "trajectory_elapsed_hours": episode_elapsed_hours,
        ...
        scene_entry[str(episode_id)] = {
            ...
            "trajectory_elapsed_hours": float(episode_elapsed_hours),
            "valid_for_summary": valid_for_summary,
            "instruction": instruction,
            "timestamp": time.time(),
        }
        ...
        summary = {
            ...
            "sum_trajectory_elapsed_hours": summary_totals["trajectory_elapsed_hours"],
        }
    ...
    if torch.distributed.is_initialized():
        ...
        if rank == 0:
            ...
            sum_elapsed_hours = sum(r.get("trajectory_elapsed_hours", 0.0) for r in valid_results)
            ...
            out_jsonl = os.path.join(args.out_dir, f"traj_eval_{ts}.jsonl")
            ...
            metrics_lines = [
                ...
                f"Total Trajectory Time (h): {sum_elapsed_hours:.4f}",
            ]
            ...
            habitat_logger.info(f"[Traj Eval] JSONL results saved to {out_jsonl}")
        ...
        sum_elapsed_hours = sum(r.get("trajectory_elapsed_hours", 0.0) for r in valid_episode_results)
...
def parse_vlm_args():
    ...
    parser.add_argument(
        "--use_image_type_embeddings",
        type=str2bool,
        default=None,
        help="Override checkpoint config for image type embeddings at inference",
    )
    parser.add_argument(
        "--use_history_temporal_embeddings",
        type=str2bool,
        default=None,
        help="Override checkpoint config for history temporal embeddings at inference",
    )
    parser.add_argument(
        "--use_padding_embeddings",
        type=str2bool,
        default=None,
        help="Override checkpoint config for padding embeddings at inference",
    )
    parser.add_argument(
        "--use_padding_embeedings",
        type=str2bool,
        default=None,
        dest="use_padding_embeddings",
        help="Deprecated alias of --use_padding_embeddings",
    )
    parser.add_argument(
        "--use_padding_embeeding",
        type=str2bool,
        default=None,
        dest="use_padding_embeddings",
        help="Deprecated alias of --use_padding_embeddings",
    ...
    parser.add_argument(
        "--use_trajectory_embeddings",
        type=str2bool,
        default=None,
        help="Override checkpoint config for trajectory embeddings at inference",
    )
    parser.add_argument(
        "--conv-style",
        type=str,
        default=None,
        help="Override checkpoint conversation template at inference, e.g. internvl2_5_nav",
    )
...
def main(cfg: DictConfig):
    ...
    global VLM_ARGS, WRITE_VISUALIZE
    args = VLM_ARGS  # Use pre-parsed args
    if args.no_vis:
        WRITE_VISUALIZE = False
    ...
    model, tokenizer = load_GO2PIXEL_model_and_tokenizer(args)
    model.eval()
    image_size = model.config.force_image_size or model.config.vision_config.image_size
    if args.use_trajectory_embeddings is None:
        args.use_trajectory_embeddings = bool(getattr(model.config, "use_history_trajectory_embeddings", False))
    habitat_logger.info(
        "[Traj Eval] embedding flags after checkpoint/CLI merge: "
        f"use_image_type_embeddings={getattr(model.config, 'use_image_type_embeddings', False)}, "
        f"use_history_temporal_embeddings={getattr(model.config, 'use_history_temporal_embeddings', False)}, "
        f"use_padding_embeddings={getattr(model.config, 'use_padding_embeddings', False)}, "
        f"use_history_trajectory_embeddings={getattr(model.config, 'use_history_trajectory_embeddings', False)}, "
        f"build_history_traj_mask={args.use_trajectory_embeddings}"
    )
