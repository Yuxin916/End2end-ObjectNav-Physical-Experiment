...
INTERNVL_NAV_SYSTEM_MESSAGE = (
    'You are an indoor navigation agent following a language instruction. '
    'Given the current egocentric image and optional history images, output exactly one normalized pixel coordinate pair '
    'in the format XXX, YYY. X and Y are integers in [0, 999]. '
    'Special actions use fixed coordinates: TURN_LEFT=(000, 500), TURN_RIGHT=(999, 500), STOP=(500, 999). '
    'For forward motion, choose the best visible navigable target in the current image, preferably far along the route. '
    'If no clear forward target is visible, output the left or right turn coordinate. '
    'Output STOP only when the agent is within 1.5 meters of the destination.'
)
...
register_conv_template(
    Conversation(
        name='internvl2_5_nav',
        system_template='<|im_start|>system\n{system_message}',
        system_message=INTERNVL_NAV_SYSTEM_MESSAGE,
        roles=('<|im_start|>user\n', '<|im_start|>assistant\n'),
        sep_style=SeparatorStyle.MPT,
        sep='<|im_end|>\n',
    )
