...
class PackedDataset(IterableDataset):
    ...
    def split_buffer(buffer, max_tokens, img_start_token_id, img_token_id, img_end_token_id):
        ...
        def _split(sample_to_split, left_idx, right_idx, left_img_idx, right_img_idx):
            ...
            for k in sample_to_split:
                if k in ['input_ids', 'labels', 'loss_weight', 'attention_mask', 'position_ids', 'data_index', 'type_ids']:
                    left_sample[k] = sample_to_split[k][:left_idx]
                    if right_sample is not None:
                        right_sample[k] = sample_to_split[k][right_idx:]
                elif k in ['pixel_values', 'image_flags', 'image_type_ids', 'history_traj_mask', 'pixel_labels']:
    ...
    def pad_buffer(self, buffer):
        ...
        pad_image_type_ids = torch.zeros(num_pad_images, dtype=torch.long)
        ...
        if 'image_type_ids' in buffer:
            buffer['image_type_ids'] = torch.cat([buffer['image_type_ids'], pad_image_type_ids])
        if 'history_traj_mask' in buffer:
            pad_history_traj_mask = torch.zeros(
                (num_pad_images, buffer['history_traj_mask'].shape[-1]),
                dtype=buffer['history_traj_mask'].dtype,
            )
            buffer['history_traj_mask'] = torch.cat([buffer['history_traj_mask'], pad_history_traj_mask])
    ...
    def get_cu_seqlens_and_indexes(
        ...
        # NOTE: per-sub-sample loss weighting (`len2weight`) is now applied inside
        # `packed_collate_fn` directly on `feat['loss_weight']`. This function only
        # produces cu_seqlens / per-sample positional indexes for FlashAttention.
        indexes = []
        cu_seqlens = [0]
...
def packed_collate_fn(
    ...
    for feat_idx, feat in enumerate(features):
        ...
        curr_cu_seqlens, curr_indexes = PackedDataset.get_cu_seqlens_and_indexes(
            data_index=data_index,
        )
        if 'loss_weight' not in feat:
            curr_labels = feat['labels']
            feat['loss_weight'] = torch.where(
                curr_labels == IGNORE_TOKEN_ID,
                torch.zeros_like(curr_labels, dtype=torch.float32),
                torch.ones_like(curr_labels, dtype=torch.float32),
            )

        # Apply per-sub-sample weighting so loss_reduction (token/sample/square) actually takes effect.
        # Without this multiplication every supervised token contributes equally and long sequences dominate.
        feat['loss_weight'] = feat['loss_weight'].to(dtype=torch.float32)
        sub_sample_min = int(data_index.min().item())
        sub_sample_max = int(data_index.max().item())
        for sub_sample_idx in range(sub_sample_min, sub_sample_max + 1):
            sample_mask = (data_index == sub_sample_idx)
            num_tokens = int(sample_mask.sum().item())
            if num_tokens <= 0:
                continue
            sample_weight = float(len2weight(num_tokens))
            feat['loss_weight'][sample_mask] = feat['loss_weight'][sample_mask] * sample_weight
    ...
    batch['loss_weight'] = torch.where(
        batch['labels'] == IGNORE_TOKEN_ID,
        torch.zeros_like(batch['loss_weight']),
        batch['loss_weight'],
    ).to(torch.float32)
