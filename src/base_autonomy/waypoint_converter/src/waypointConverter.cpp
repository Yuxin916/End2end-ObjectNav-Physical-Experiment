...
std_msgs::msg::Float32 waypointReachedMsgs;
geometry_msgs::msg::PointStamped waypointMsgs;

rclcpp::Publisher<std_msgs::msg::Float32>::SharedPtr pubWaypointReachedPtr;
...
void readBoundaryFile()
{
  ...
  if (boundary_file == NULL) {
    sendBoundary = false;
    RCLCPP_INFO(nh->get_logger(), "Cannot read boundary.ply file.");
    return;
  }
...
void poseHandler(const nav_msgs::msg::Odometry::ConstSharedPtr pose)
{
  ...
  if (dis < waypointXYRadius) {
    if (!waypointReached) {
      float disX2 = vehicleX - waypointX;
      float disY2 = vehicleY - waypointY;
      float dis2 = sqrt(disX2 * disX2 + disY2 * disY2);

      waypointReachedMsgs.data = dis2;
      pubWaypointReachedPtr->publish(waypointReachedMsgs);

      RCLCPP_INFO(nh->get_logger(), "Waypoint reached.");
    }
  ...
  if (waypointReached) {
    ...
    for (int ind = 0; ind < pointSearchNum; ind++) {
      ...
      float dis2 = sqrt(disX2 * disX2 + disY2 * disY2) + vehicleDisWeight * sqrt(pointSearchSqDis[ind]);
...
void waypointHandler(const geometry_msgs::msg::Pose2D::ConstSharedPtr waypoint)
{
  if (checkTravArea) {
    ...
    if (pointSearchSqDis[0] > disThre * disThre && !useTerrainAnalysis) {
      RCLCPP_INFO(nh->get_logger(), "Waypoint not in traversable area, skip.");
      return;
    }
  }
...
int main(int argc, char** argv)
{
  ...
  nh = rclcpp::Node::make_shared("waypointConverter");
  ...
  nh->declare_parameter<double>("vehicleDisWeight", vehicleDisWeight);
  ...
  nh->get_parameter("vehicleDisWeight", vehicleDisWeight);
  ...
  auto pubWaypointReached = nh->create_publisher<std_msgs::msg::Float32> ("/way_point_reached", 5);
  pubWaypointReachedPtr = pubWaypointReached;
  ...
  if (checkTravArea && !useTerrainAnalysis) {
    ...
    if (ply_reader.read(trav_area_file_dir, *travArea) == -1) {
      checkTravArea = false;
      RCLCPP_INFO(nh->get_logger(), "Couldn't read traversable_area.ply file.");
    } else {
