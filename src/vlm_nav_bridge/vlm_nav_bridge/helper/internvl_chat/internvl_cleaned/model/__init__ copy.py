...
def load_model_and_tokenizer(args):
    ...
    # Some checkpoints can leave newly introduced adapter params as meta tensors
    # (e.g. cand_pos_mlp.*). Materialize them on CPU before any .cuda() call.
    meta_param_names = [n for n, p in model.named_parameters() if getattr(p, 'is_meta', False)]
    if meta_param_names:
        for full_name in meta_param_names:
            module_name, param_name = full_name.rsplit('.', 1)
            module = model.get_submodule(module_name) if module_name else model
            old_param = getattr(module, param_name)
            new_data = torch.empty(old_param.shape, dtype=old_param.dtype, device='cpu')
            if new_data.ndim > 1:
                torch.nn.init.normal_(new_data, mean=0.0, std=0.02)
            else:
                torch.nn.init.zeros_(new_data)
            setattr(module, param_name, torch.nn.Parameter(new_data, requires_grad=old_param.requires_grad))
