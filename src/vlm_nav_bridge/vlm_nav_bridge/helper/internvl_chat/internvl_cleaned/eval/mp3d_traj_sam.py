...
def translate_objnav(object_goal, hm3d_mode=False):
    ...
    if hm3d_mode:
        ...
        if target in ["chair", "bench", "stool", "desk"]:
            ...
            confusing_target_list = ["tv_monitor", "mirror", "mirror_frame", "oven", "microwave", "wall"]
        else:
...
def evaluate_trajectories(vlm_policy, object_perceiver: MMDINOSAM_Perceiver, config):
    ...
    for idx, ep_info in enumerate(episodes_to_run):
            ...
            if not skip_step:
                ...
                if not target_found and current_frame_has_target and target_temporally_consistent:
                    ...
                    try:
                        ...
                        # Guard: Open3D crashes (GPU OOM or CPU assertion) on empty PCD in voxel_down_sample.
                        if target_pcd_nw.is_empty():
                            target_pcd_nw = None
                        else:
                        ...
                        # GPU OOM during target PCD processing (GPU full from BEV mapping).
                        # Fall back to CPU: target PCD is small (one frame) so CPU is acceptable.
                        habitat_logger.warning(f"GPU OOM during target PCD at step {save_idx}; retrying on CPU.")
                        torch.cuda.empty_cache()
                        try:
                            o3d.core.cuda.release_cache()
                        except Exception:
                            pass
                        # Explicitly delete the GPU PCD before CPU path to release GPU reference.
                        try:
                            del target_pcd_nw
                        except Exception:
                            pass
                        try:
                            _cpu_dev = o3d.core.Device('CPU:0')
                            target_pcd_nw = gpu_pointcloud_from_array(target_pcd_world, np.ones_like(target_pcd_world), _cpu_dev)
                            if target_pcd_nw.is_empty():
                                target_pcd_nw = None
                            else:
                                target_pcd_nw = target_pcd_nw.voxel_down_sample(BEV_map.pcd_resolution)
                            ...
                            habitat_logger.warning(f"CPU fallback also failed ({_cpu_err}); skipping target at step {save_idx}.")
                            target_pcd_nw = None
                    if target_pcd_nw is None:
                        ...
                        if target_position_3d[0, 2] < BEV_map.current_agent_world_position[2] - 0.5:
                            ...
                            # Walk target toward agent until navigable
                            ...
                            if np.linalg.norm(target_position_3d[0, :2] - B[0, :2]) > 1.0:
        ...
        # torch.cuda.empty_cache() only flushes PyTorch's pool; Open3D has its own
        # caching allocator that must be flushed separately to avoid OOM after many episodes.
        torch.cuda.empty_cache()
        try:
            o3d.core.cuda.release_cache()
        except Exception:
