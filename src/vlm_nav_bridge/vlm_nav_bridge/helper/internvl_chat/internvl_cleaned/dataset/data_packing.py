...
from internvl_cleaned.constants_utils import IMG_CONTEXT_TOKEN, IMG_END_TOKEN, IMG_START_TOKEN
...
class PackedDataset(IterableDataset):
    ...
    def next_data(self, current_dataset_idx):
        while True:
            try:
                ...
                logger.exception(
                    f"[worker_id={self.worker_id}] next_data() error on "
                    f"ds_idx={current_dataset_idx}, ds_name={self.datasets[current_dataset_idx].ds_name if self.datasets else 'N/A'}"
                )
                if len(self.datasets) == 0:
                    raise StopIteration
                current_dataset_idx = np.random.choice(len(self.datasets))
    ...
    def update_buffer(self, buffer, new_sample):
        if buffer is None:
            ...
            # Convert position_info from dict to list for consistency with packed data structure
            if 'position_info' in new_sample and new_sample['position_info'] is not None:
                if isinstance(new_sample['position_info'], dict):
                    new_sample['position_info'] = [new_sample['position_info']]
                # If it's already a list, keep it as is
            else:
                new_sample['position_info'] = [None]
            return new_sample

        new_sample['data_index'] = torch.ones_like(new_sample['input_ids']) + buffer['data_index'][-1].item()

        # Handle tensor keys that can be concatenated
        tensor_keys = ['input_ids', 'labels', 'attention_mask', 'position_ids', 'data_index', 'type_ids', 'pixel_values', 'image_flags']
        if 'loss_weight' in buffer:
            tensor_keys.append('loss_weight')
        if 'image_token_counts' in buffer and 'image_token_counts' in new_sample:
            tensor_keys.append('image_token_counts')

        # Check that tensor keys match (excluding position_info which is handled separately)
        buffer_tensor_keys = set(k for k in buffer.keys() if k in tensor_keys and k != 'position_info')
        new_tensor_keys = set(k for k in new_sample.keys() if k in tensor_keys and k != 'position_info')
        assert buffer_tensor_keys == new_tensor_keys, f"Tensor keys mismatch: buffer={buffer_tensor_keys}, new={new_tensor_keys}"

        # Concatenate tensor keys
        for k in tensor_keys:
            if k in buffer:
                buffer[k] = torch.cat([buffer[k], new_sample[k]])

        # Handle position_info: for packed data, we keep a list of position_info dicts
        # One per sub-sample in the buffer
        if 'position_info' not in buffer:
            buffer['position_info'] = []
        elif not isinstance(buffer['position_info'], list):
            # If buffer['position_info'] is a dict (from first sample), convert to list
            buffer['position_info'] = [buffer['position_info']]

        if 'position_info' in new_sample and new_sample['position_info'] is not None:
            # Ensure new_sample['position_info'] is a dict (not a list) before appending
            if isinstance(new_sample['position_info'], list):
                # If it's already a list, extend (shouldn't happen, but handle it)
                buffer['position_info'].extend(new_sample['position_info'])
            else:
                buffer['position_info'].append(new_sample['position_info'])
        else:
            buffer['position_info'].append(None)

        return buffer

    @staticmethod
    def check_valid(sample_to_check, min_active_tokens=1):
        # Frontier-index supervision can be as short as one token (e.g., "3").
        # A ratio-only filter (e.g., >1/256) over-prunes valid long prompts and can lead to zero-loss training.
        num_active_tokens = (sample_to_check['labels'] != IGNORE_TOKEN_ID).sum().item()
        return num_active_tokens >= min_active_tokens
    ...
    def split_buffer(buffer, max_tokens, img_start_token_id, img_token_id, img_end_token_id):
        ...
        def _split(sample_to_split, left_idx, right_idx, left_img_idx, right_img_idx):
            ...
            for k in sample_to_split:
                ...
                elif k == 'image_token_counts':
                    if k in sample_to_split:
                        left_sample[k] = sample_to_split[k][:left_img_idx]
                        if right_sample is not None:
                            right_sample[k] = sample_to_split[k][right_img_idx:]
                elif k == 'position_info':
                    # position_info is a list of dicts (one per sub-sample in packed buffer)
                    # We need to split it based on which sub-samples go to left vs right
                    # This is determined by data_index values in the split
                    if 'position_info' in sample_to_split and sample_to_split['position_info'] is not None:
                        if isinstance(sample_to_split['position_info'], list):
                            # Determine which sub-samples are in left vs right based on data_index
                            # Get unique data_index values in left and right splits
                            left_data_indices = set(sample_to_split['data_index'][:left_idx].unique().tolist())
                            if right_sample is not None:
                                right_data_indices = set(sample_to_split['data_index'][right_idx:].unique().tolist())
                            else:
                                right_data_indices = set()

                            # Split position_info list based on data_index
                            left_pos_info = []
                            right_pos_info = []
                            for idx, pos_info in enumerate(sample_to_split['position_info']):
                                if idx in left_data_indices:
                                    left_pos_info.append(pos_info)
                                elif idx in right_data_indices:
                                    right_pos_info.append(pos_info)

                            left_sample[k] = left_pos_info if left_pos_info else None
                            if right_sample is not None:
                                right_sample[k] = right_pos_info if right_pos_info else None
                        else:
                            # Single dict (not packed), assign to left
                            left_sample[k] = sample_to_split['position_info']
                            if right_sample is not None:
                                right_sample[k] = None
                else:
                    # Preserve other keys (like loss_weight, etc.) by copying to left
                    # TODO: Handle other metadata keys properly when splitting
                    if k not in ['worker_state_key', 'worker_state_dict', 'custom_infos']:
                        left_sample[k] = sample_to_split[k]
                        if right_sample is not None:
                            right_sample[k] = sample_to_split[k]  # Copy for now
    ...
    def pad_buffer(self, buffer):
        ...
        if 'image_token_counts' in buffer:
            pad_counts = torch.zeros(num_pad_images, dtype=buffer['image_token_counts'].dtype, device=buffer['image_token_counts'].device)
            buffer['image_token_counts'] = torch.cat([buffer['image_token_counts'], pad_counts])
...
def packed_collate_fn(
    ...
    for feat_idx, feat in enumerate(features):
        ...
        # Note: Keep in feat for collator (needed for position embedding alignment)
        data_index = feat['data_index']
        ...
        # Extract position_info if present (don't pop, let data_collator handle it)
        # position_info will be preserved by concat_pad_data_collator
    ...
    batch.pop('type_ids')
    logger.info(
        f"[packed_collate_fn] packed {num_samples} samples, "
        f"added {num_padding_tokens} pad tokens, "
        f"added {batch['image_flags'].numel() - batch['image_flags'].sum().item()} pad images"
    )
    return batch
