...
def preprocess_internvl2_5(
    ...
    if not text_only:
        new_conversations = []
        current_image_idx = 0
        use_bev_ego_placeholders = '<image_bev>' in str(conversations) or '<image_ego>' in str(conversations)
        for conversation in conversations:
            if conversation['from'] == 'human':
                if use_bev_ego_placeholders:
                    # Replace <image_bev> and <image_ego> in document order with num_image_token_list
                    value = conversation['value']
                    while current_image_idx < num_image:
                        pos_bev = value.find('<image_bev>')
                        pos_ego = value.find('<image_ego>')
                        if pos_bev == -1 and pos_ego == -1:
                            break
                        token_count = num_image_token_list[current_image_idx]
                        image_tokens = f'{IMG_START_TOKEN}{IMG_CONTEXT_TOKEN * token_count}{IMG_END_TOKEN}'
                        if pos_bev >= 0 and (pos_ego == -1 or pos_bev <= pos_ego):
                            value = value.replace('<image_bev>', image_tokens, 1)
                        else:
                            value = value.replace('<image_ego>', image_tokens, 1)
                        current_image_idx += 1
    ...
    for role, input_id in zip(roles, input_ids):
        ...
        if role == 'system' or role == 'human':
            ...
            # Keep at least one supervised token for very short answers (e.g., "5").
            # Unconditionally masking the final token can turn the whole target into IGNORE
            # when tokenizer merges trailing markers/newline into a short sequence.
            if target.shape[0] > ignore_len + 1:
                target[-1:] = IGNORE_TOKEN_ID  # ignore trailing `\n`
